...
*Setup instructions for a Gitian build of DigiCoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the DigiCoin
...
- [Building DigiCoin Core](#building-digicoin-core)
...
Clone the git repositories for digicoin and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/digicoinofficial/DigiCoin-Core --recursive
git clone https://github.com/digicoinofficial/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/digicoin/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the digicoin repository under 'Fetch and create inputs' to install sources which require
...
To build DigiCoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the digicoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/digicoin/.git/
...
For example:
```bash
URL=https://github.com/laanwj/digicoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit digicoin=${COMMIT} --url digicoin=${URL} ../digicoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit digicoin=${COMMIT} --url digicoin=${URL} ../digicoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit digicoin=${COMMIT} --url digicoin=${URL} ../digicoin/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-digicoin:
./bin/gbuild --commit digicoin=${COMMIT},cpp-eth-digicoin=${ETHCOMMIT} --url digicoin=${URL},cpp-eth-digicoin=${ETHURL} ../digicoin/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the digicoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../digicoin/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/digicoinofficial/DigiCoin-Core-detached-sigs.git

BTCPATH=/some/root/path/digicoin
SIGPATH=/some/root/path/digicoin-detached-sigs

./bin/gbuild --url digicoin=${BTCPATH},signature=${SIGPATH} ../digicoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/digicoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/digicoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/digicoin-osx-build.assert
```
...
[digicoinproject/gitian.sigs](https://github.com/digicoinofficial/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@digicoin.org) and he will commit them.