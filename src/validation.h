...
#include <digicoin/storageresults.h>


extern std::unique_ptr<DigiCoinState> globalState;
...
using ExtractDigiCoinTX = std::pair<std::vector<DigiCoinTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // digicoin
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashDigiCoinRoot,
    const dev::h256& oldHashStateRoot, const std::vector<DigiCoinTransaction>& transactions);
...
class DigiCoinTxConverter{

public:

    DigiCoinTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionDigiCoinTransactions(ExtractDigiCoinTX& digicoinTx);
...
private:
    ...
    DigiCoinTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<DigiCoinTransaction> _txs, const uint64_t _blockGasLimit, CBlockIndex* _pindex) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit), pindex(_pindex) {}
