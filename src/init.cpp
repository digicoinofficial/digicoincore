...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/digicoinofficial/DigiCoin-Core>";
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                dev::eth::Ethash::init();
                fs::path digicoinStateDir = GetDataDir() / "stateDigiCoin";
                bool fStatus = fs::exists(digicoinStateDir);
                const std::string dirDigiCoin(digicoinStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsDigiCoinstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<DigiCoinState>(new DigiCoinState(dev::u256(0), DigiCoinState::openDB(dirDigiCoin, hashDB, dev::WithExisting::Trust), dirDigiCoin, existsDigiCoinstate));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::digicoinMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(digicoinStateDir.string()));
                if (fReset) {
