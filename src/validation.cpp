...
std::unique_ptr<DigiCoinState> globalState;
...
bool fAddressIndex = false; // digicoin
#endif
...
const std::string strMessageMagic = "DigiCoin Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool test_accept, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            uint64_t minGasPrice = digicoinDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = digicoinDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            DigiCoinTxConverter converter(tx, NULL);
            ExtractDigiCoinTX resultConverter;
            if(!converter.extractionDigiCoinTransactions(resultConverter)){
            ...
            std::vector<DigiCoinTransaction> digicoinTransactions = resultConverter.first;
            std::vector<EthTransactionParams> digicoinETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(DigiCoinTransaction digicoinTransaction : digicoinTransactions){
                sumGas += digicoinTransaction.gas() * digicoinTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, digicoinTransaction.gasPrice());
                } else {
                    txMinGasPrice = digicoinTransaction.gasPrice();
                }
                VersionVM v = digicoinTransaction.getVersion();
                if(v.format!=0)
                ...
                if(digicoinTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(digicoinTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(digicoinTransaction.gas() > UINT32_MAX)
                ...
                gasAllTxs += digicoinTransaction.gas();
                ...
                if(v.rootVM!=0 && (uint64_t)digicoinTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(digicoinETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > digicoinTransactions.size())
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // digicoin
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // digicoin
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    DigiCoinTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());


    ByteCodeExec exec(block, std::vector<DigiCoinTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path digicoinDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(digicoinDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(digicoinDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(DigiCoinTransaction& tx : txs){
        //validate VM version
...
bool DigiCoinTxConverter::extractionDigiCoinTransactions(ExtractDigiCoinTX& digicointx){
    std::vector<DigiCoinTransaction> resultTX;
    ...
    digicointx = std::make_pair(resultTX, resultETP);
    return true;
}

bool DigiCoinTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool DigiCoinTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
DigiCoinTransaction DigiCoinTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    DigiCoinTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = DigiCoinTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = DigiCoinTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // digicoin
    DigiCoinDGP digicoinDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setDigiCoinSchedule(digicoinDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = digicoinDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = digicoinDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = digicoinDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // digicoin
...
///////////////////////////////////////////////////////////////////////////////////////// digicoin
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            DigiCoinTxConverter convert(tx, &view, &block.vtx);

            ExtractDigiCoinTX resultConvertDigiCoinTX;
            if(!convert.extractionDigiCoinTransactions(resultConvertDigiCoinTX)){
            ...
            if(!CheckMinGasPrice(resultConvertDigiCoinTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertDigiCoinTX.first, blockGasLimit, pindex->pprev);
            ...
            for(DigiCoinTransaction& qtx : resultConvertDigiCoinTX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertDigiCoinTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertDigiCoinTX.first[k].from(), resultConvertDigiCoinTX.first[k].to(),
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // digicoin
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // digicoin
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // digicoin
            globalState->setRootUTXO(oldHashUTXORoot); // digicoin
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // digicoin
        uint32_t sizeBlockDGP = digicoinDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for DigiCoin PoS we need to sync this after every block to ensure txdb is populated for
