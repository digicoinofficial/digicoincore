...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setDigiCoinSchedule(digicoinDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = digicoinDGP.getBlockSize(nHeight);
    minGasPrice = digicoinDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = digicoinDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    DigiCoinTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractDigiCoinTX resultConverter;
    if(!convert.extractionDigiCoinTransactions(resultConverter)){
    ...
    for(DigiCoinTransaction digicoinTransaction : digicoinTransactions){
        txGas += digicoinTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + digicoinTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(digicoinTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, digicoinTransactions, hardBlockGasLimit, chainActive.Tip());
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "digicoinstake";
...
void StakeDigiCoins(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
