...
static UniValue addnode(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 2 ||
        ...
        throw std::runtime_error(
            ...
            "\nExamples - Mainnet port 3108, Testnet port 13108, Regtest port 23108:\n"
            + HelpExampleCli("addnode", "\"192.168.0.6:3108\" \"onetry\"")
            + HelpExampleRpc("addnode", "\"192.168.0.6:3108\", \"onetry\"")
        );
...
static UniValue disconnectnode(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() == 0 || request.params.size() >= 3)
        throw std::runtime_error(
            ...
            "\nExamples - Mainnet port 3108, Testnet port 13108, Regtest port 23108:\n"
            + HelpExampleCli("disconnectnode", "\"192.168.0.6:3108\"")
            + HelpExampleCli("disconnectnode", "\"\" 1")
            + HelpExampleRpc("disconnectnode", "\"192.168.0.6:3108\"")
...
static UniValue getaddednodeinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "       {\n"
            "         \"address\" : \"192.168.0.201:3108\",  (string) The digicoin server IP and port we're connected to\n"
