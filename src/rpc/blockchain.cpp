...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // digicoin
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // digicoin
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // digicoin
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // digicoin
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() >= 3){
        CTxDestination digicoinSenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(digicoinSenderAddress)) {
            const CKeyID *keyid = boost::get<CKeyID>(&digicoinSenderAddress);
...
UniValue gettxout(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of digicoin addresses\n"
