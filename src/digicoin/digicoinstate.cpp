...
#include <digicoin/digicoinstate.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

DigiCoinState::DigiCoinState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = DigiCoinState::openDB(_path + "/digicoinDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

DigiCoinState::DigiCoinState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute DigiCoinState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, DigiCoinTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createDigiCoinAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            digicoin::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> DigiCoinState::vins() const // temp
{
...
void DigiCoinState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* DigiCoinState::vin(dev::Address const& _a) const
{
    return const_cast<DigiCoinState*>(this)->vin(_a);
}

Vin* DigiCoinState::vin(dev::Address const& _addr)
{
...
// void DigiCoinState::commit(CommitBehaviour _commitBehaviour)
// {
...
void DigiCoinState::kill(dev::Address _addr)
{
...
void DigiCoinState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void DigiCoinState::deleteAccounts(std::set<dev::Address>& addrs){
...
void DigiCoinState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void DigiCoinState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
