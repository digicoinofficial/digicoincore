#include <digicoin/digicoinDGP.h>

void DigiCoinDGP::initDataEIP158(){
...
bool DigiCoinDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule DigiCoinDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t DigiCoinDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t DigiCoinDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t DigiCoinDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t DigiCoinDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool DigiCoinDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void DigiCoinDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void DigiCoinDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void DigiCoinDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void DigiCoinDGP::createParamsInstance(){
...
dev::Address DigiCoinDGP::getAddressForBlock(unsigned int blockHeight){
...
void DigiCoinDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void DigiCoinDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void DigiCoinDGP::parseStorageOneUint64(uint64_t& value){
...
void DigiCoinDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule DigiCoinDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
