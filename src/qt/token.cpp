...
bool ToHash160(const std::string& strDigiCoinAddress, std::string& strHash160)
{
    CTxDestination digicoinAddress = DecodeDestination(strDigiCoinAddress);
    if(!IsValidDestination(digicoinAddress))
        return false;
    const CKeyID * keyid = boost::get<CKeyID>(&digicoinAddress);
    if(keyid){
...
bool ToDigiCoinAddress(const std::string& strHash160, std::string& strDigiCoinAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CTxDestination digicoinAddress = keyid;
    if(IsValidDestination(digicoinAddress)){
        strDigiCoinAddress = EncodeDestination(digicoinAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToDigiCoinAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToDigiCoinAddress(tokenEvent.receiver, tokenEvent.receiver);
