#include <qt/digatoken.h>
#include <qt/forms/ui_digatoken.h>
...
Token::Token(const PlatformStyle *platformStyle, QWidget *parent) :
    QWidget(parent),
    ui(new Ui::Token),
    m_model(0),
    m_clientModel(0),
...
void Token::setModel(WalletModel *_model)
{
    m_model = _model;
...
void Token::setClientModel(ClientModel *_clientModel)
{
...
void Token::on_goToSendTokenPage()
{
    ui->stackedWidgetToken->setCurrentIndex(0);
}

void Token::on_goToReceiveTokenPage()
{
    ui->stackedWidgetToken->setCurrentIndex(1);
}

void Token::on_goToAddTokenPage()
{
    ui->stackedWidgetToken->setCurrentIndex(2);
}

void Token::on_currentTokenChanged(QModelIndex index)
{
    if(m_tokenModel)
    {
        if(index.isValid())
...
void Token::on_dataChanged(const QModelIndex &topLeft, const QModelIndex &bottomRight, const QVector<int> &roles)
{
...
void Token::on_currentChanged(QModelIndex current, QModelIndex previous)
{
    Q_UNUSED(previous);
...
void Token::on_rowsInserted(QModelIndex index, int first, int last)
{
    Q_UNUSED(index);
    Q_UNUSED(first);
    Q_UNUSED(last);
...
void Token::contextualMenu(const QPoint &point)
{
