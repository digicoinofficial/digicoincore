...
#include <qt/digicoinversionchecker.h>
    ...
    platformStyle(_platformStyle)
{
    ...
    digicoinVersionChecker = new DigiCoinVersionChecker(this);
...
void BitcoinGUI::createActions()
{
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and digicoin: URIs)"));
    ...
    TokenAction = new QAction(platformStyle->MultiStatesIcon(":/icons/digatoken"), tr("& Tokens"), this);
    TokenAction->setStatusTip(tr(" Tokens (send, receive or add Tokens in list)"));
    TokenAction->setToolTip(TokenAction->statusTip());
    TokenAction->setCheckable(true);
    TokenAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_6));
    tabGroup->addAction(TokenAction);
    ...
    openAction->setStatusTip(tr("Open a digicoin: URI or payment request"));//_LINE

    showHelpMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible DigiCoin command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        appNavigationBar->mapGroup(TokenAction, tokenActions);
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && digicoinVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(_RELEASES, _RELEASES);
            QString message(tr("New version of DigiCoin wallet is available on the DigiCoin source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to DigiCoin network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
