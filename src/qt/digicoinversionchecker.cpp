...
#define paternVersion "digicoin-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

DigiCoinVersionChecker::DigiCoinVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

DigiCoinVersionChecker::~DigiCoinVersionChecker()
{

}

bool DigiCoinVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> DigiCoinVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(_RELEASES)));
    QEventLoop event;
...
Version DigiCoinVersionChecker::getMaxReleaseVersion()
{
