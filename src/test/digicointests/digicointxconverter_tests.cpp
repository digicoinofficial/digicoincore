...
void checkResult(bool isCreation, std::vector<DigiCoinTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    DigiCoinTxConverter converter(transaction, NULL);
    ExtractDigiCoinTX digicoinTx;
    BOOST_CHECK(converter.extractionDigiCoinTransactions(digicoinTx));
    std::vector<DigiCoinTransaction> result = digicoinTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(digicointxconverter_tests, TestingSetup)
