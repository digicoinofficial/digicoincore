...
inline void initState(){
    ...
    const std::string dirDigiCoin = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<DigiCoinState>(new DigiCoinState(dev::u256(0), DigiCoinState::openDB(dirDigiCoin, hashDB, dev::WithExisting::Trust), dirDigiCoin + "/digicoinDB", dev::eth::BaseState::Empty));
...
inline dev::Address createDigiCoinAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline DigiCoinTransaction createDigiCoinTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    DigiCoinTransaction txEth;
    if(recipient == dev::Address()){
        txEth = DigiCoinTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = DigiCoinTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<DigiCoinTransaction> txs){
